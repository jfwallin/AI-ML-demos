...
# Format the similarity scores to 2 decimal places
...
# Apply conditional formatting to highlight values over 0.80
def highlight_high_similarity(val):
    color = 'background-color: yellow' if val > 0.80 else ''
    return color

styled_df = similarity_df.style.applymap(highlight_high_similarity)

# Save the styled DataFrame to an HTML file for better visualization
output_html_path = '/Users/ian/cosine_similarity_matrix.html'
styled_df.to_html(output_html_path)
print(f"Styled similarity matrix saved to {output_html_path}")